...
@section('title', 'Buat Review Outlet - ' . $outlet->name)
            ...
            <div class="card shadow">
                <div class="card-header bg-warning text-dark">
                    <div class="d-flex align-items-center">
                        ...
                        <h5 class="mb-0">Buat Review Outlet</h5>
                    </div>
                </div>
                
                <div class="card-body">
                    <!-- Outlet Info -->
                    <div class="outlet-info mb-4 p-3 bg-light rounded">
                        <div class="d-flex align-items-center">
                            @if($outlet->image)
                                <img src="{{ Storage::url($outlet->image) }}" 
                                     alt="{{ $outlet->name }}" 
                                     class="rounded me-3" 
                    ...
                    <!-- Review Form -->
                    <form action="{{ route('reviews.store.outlet', $outlet) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('comment') is-invalid @enderror" 
                                      ...
                                      name="comment" 
                                      rows="4" 
                                      placeholder="Bagikan pengalaman Anda dengan outlet ini...">{{ old('comment') }}</textarea>
                            @error('comment')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
